...
test('1180x820 tablet landscape keeps Station Controls and neighboring grid boundaries flush', { timeout: 20000 }, async t => {
    ...
    const expression = `(() => {
      const read = selector => { const element = document.querySelector(selector); const rect = element.getBoundingClientRect(); const style = getComputedStyle(element); return { top: rect.top, bottom: rect.bottom, height: rect.height, clientHeight: element.clientHeight, scrollHeight: element.scrollHeight, overflowY: style.overflowY }; };
