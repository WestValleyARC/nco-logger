...
test('responsive logger keeps independent orientation layouts and touch-safe controls', () => {
    ...
    assert.match(css, /\[data-layout-context\^="phone"\] \.nch-entry-controls\s*\{[^}]*padding-bottom:\s*calc\(12px \+ env\(safe-area-inset-bottom\)\)[^}]*overflow-y:\s*auto[^}]*overscroll-behavior-y:\s*contain/s);
    ...
    assert.match(css, /\[data-layout-context="tabletLandscape"\] \.nch-controls-pane \.nch-entry-controls\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1fr\) minmax\(0, 1\.35fr\) minmax\(0, 1fr\)/s);
