...
test('responsive logger keeps independent orientation layouts and touch-safe controls', () => {
    ...
    assert.match(source, /phonePortrait:[\s\S]*controls: \{ x: 0, y: 0, w: 24, h: 5[\s\S]*active: \{ x: 0, y: 5[\s\S]*chat: \{ x: 0, y: 19/);
    ...
    assert.match(source, /currentLayoutContext === "phonePortrait" && moduleAvailable\("controls"\) && items\.controls\.h < 5[\s\S]*items\.controls\.h = 5[\s\S]*items\[id\]\.y \+ addedRows/);
    assert.match(source, /currentLayoutContext === "phonePortrait" && id === "controls" \? 5 : MIN_MODULE_ROWS\[id\]/);
});
