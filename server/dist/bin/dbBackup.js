...
function requireUriDbname(uri, origin) {
    ...
    if (!dbname)
        throw new Error(`MongoDB URI from ${origin} must include a database name`);
    return dbname;
}
    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = () => {
            ...
            if (srcCode === 0 && dstCode === 0)
                ...
                reject(new Error(`pipe failed (src=${srcCode}, dst=${dstCode})`));
        };
        src.on('error', reject);
        dst.on('error', reject);
...
function confirmTargetWrite(target, opts) {
    ...
    if (target.environment === 'development')
        return;
    if (opts['confirm-target'] === target.dbname)
        return;
...
async function cmdMigrate(opts) {
    ...
    if (optionBoolean(opts['oplog']) && !useOplog) {
        ...
        if (remap.length)
            reasons.push('namespaces differ');
...
async function verifyParity(srcUri, tgtUri, readPreference = 'primaryPreferred') {
    ...
    for (const n of names) {
        ...
        if (s !== t || sIdx !== tIdx)
            mismatch++;
        console.log(`${n.padEnd(30)} ${String(s).padStart(8)} ${String(t).padStart(10)} ${String(diff).padStart(6)}   ${idxOk}`);
    }
    if (mismatch) {
...
function configureCli(args) {
    void (0, yargs_1.default)(args)
        ...
        .command('restore', 'restore a gzipped archive into a database', (y) => y.options({
        ...commonConnectionOpts,
        ...
        .command('migrate', 'copy data from one URI to another (dump→restore)', (y) => y.options({
        ...sourceTargetOpts,
        ...
        .command('verify', 'compare doc counts and indexes between two URIs', (y) => y.options(sourceTargetOpts), (a) => run(cmdVerify, a))
        .command('list', 'list local + S3 backups', (y) => y.options({
        'dir': commonConnectionOpts.dir,
        's3-bucket': { type: 'string' },
        's3-prefix': { type: 'string', default: 'hamlive' }
    }), (a) => run(cmdList, a))
        .command('prune', 'delete local backups older than N days', (y) => y.options({
        'dir': commonConnectionOpts.dir,
