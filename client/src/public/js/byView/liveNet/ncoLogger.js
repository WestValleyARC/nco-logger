...
(() => {
  ...
  async function qrzLogin() {
    ...
    if (!username || !password) throw new Error("Enter your QRZ username and password under QRZ Setup.");
    ...
    if (!response.ok) throw new Error(`QRZ login failed (${response.status}).`);
    const doc = new DOMParser().parseFromString(await response.text(), "application/xml");
    const error = xmlText(doc, "Error");
    if (error) throw new Error(`QRZ: ${error}`);
    qrzSessionKey = xmlText(doc, "Key");
    if (!qrzSessionKey) throw new Error(xmlText(doc, "Message") || "QRZ did not return a session key.");
    const alert = xmlText(doc, "Alert");
    const passwordInput = panel?.querySelector("[data-role='qrz-password']");
    if (passwordInput) passwordInput.value = "";
    refreshQrzPasswordHint();
    storeQrzAuth();
    if (alert) setStatus(`QRZ alert: ${alert}`, "warning");
  }
  ...
  async function lookupQrz(callSign, allowSessionRetry = true, options = {}) {
    ...
    try {
      if (!qrzSessionKey) await qrzLogin();
      ...
      if (!response.ok) throw new Error(`QRZ lookup failed (${response.status}).`);
      const doc = new DOMParser().parseFromString(await response.text(), "application/xml");
      const error = xmlText(doc, "Error");
      if (error) {
  ...
  function queueMissingQrzPhotos() {
    if (!qrzPassword && !qrzSessionKey) return;
  ...
  async function resolvedAvatarSource(candidate) {
    ...
    if (source) resolvedAvatarDataUrls.set(candidate, source);
    if (!source) {
      window.setTimeout(() => {
  ...
  function hydrateAvatars(root = panel) {
    root?.querySelectorAll("img.nch-avatar[data-qrz-photo]").forEach(image => {
      ...
      if (!candidate || image.dataset.loading === "true" || resolvedAvatarDataUrls.get(candidate) === image.src) return;
      ...
      resolvedAvatarSource(candidate).then(source => {
        ...
        preload.onerror = () => { if (image.isConnected) image.src = DEFAULT_AVATAR; };
        preload.src = source;
      });
    });
  }
  ...
  function tryResolveGridLayout(source, fixedId = "", nudgeFixed = false) {
    ...
    const placeNext = index => {
      ...
      for (const candidate of candidates) {
        attempts += 1;
        if (candidate.x < 0 || candidate.y < 0 || candidate.x + candidate.w > GRID_COLUMNS || candidate.y + candidate.h > GRID_ROWS) continue;
  ...
  function updateModulePointerDrag(event) {
    ...
    if (!modulePointerDrag.started && distance < 6) return;
    if (!modulePointerDrag.started) {
  ...
  async function refreshOnce() {
    ...
    try {
      ...
      if (netTitle) netTitle.textContent = latestNetTitle;
      applyRoleUi();
      applyModuleLayout();
      dockNativeChat();
      customizeNativeChat();
  ...
  const qrzStorageGet = () => new Promise(resolve =>
    browserStorage.get([qrzUserKey, qrzAuthKey], resolve)
  );
