...
import {
  ...
  selectNcoAvatarSource, setBoundedCache
...
import {
  findLoggerGridItemPosition, loggerGridLayoutIsCollisionFree, replaceLoggerGridItem
} from "../../lib/loggerGrid.js";
...
(() => {
  ...
  const SLASH_COMMAND_BY_TOKEN = (() => {
    ...
    for (const command of SLASH_COMMANDS) {
      for (const token of [command.name, ...command.aliases]) {
        if (commands.has(token)) throw new Error(`Duplicate NCO Helper slash token: ${token}`);
  ...
  const selfCall = () =>
    (document.querySelector("#serverInfo")?.dataset?.callSign || "").trim().toUpperCase();
  ...
  const browserStorage = {
    get(keys, callback) {
      ...
      for (const key of keys) {
        ...
        } catch { /* Ignore malformed data from an earlier preview. */ }
      }
      callback(result);
    },
    set(entries) {
  ...
  function commandAllowed(command) {
    ...
    if (["o", "io", "ui"].includes(verb) && station?.role === "netcontrol") return false;
    if (verb === "o" && call === selfCall()) return false;
    if (isNcoUser()) return ["i", "o", "ui", "io", "l", "r", "handoff", "close", "f"].includes(verb);
    if (currentUserRole === "netlogger") return ["i", "o", "ui", "io", "r"].includes(verb);
    return false;
  }
  ...
  const formatName = value => String(value || "").trim().split(/\s+/).filter(Boolean).map(part => {
    ...
    if (["I", "II", "III", "IV", "V"].includes(suffix)) return suffix;
    if (suffix === "JR") return "Jr.";
    if (suffix === "SR") return "Sr.";
    return normalized;
  }).join(" ");
  const formatLocation = value => String(value || "").trim().split(/\s*,\s*/).filter(Boolean).map(part =>
    /^[a-z]{2,3}$/i.test(part) ? part.toLocaleUpperCase() : formatName(part)
  ).join(", ");
  ...
  const isNewerVersion = (candidate, current = VERSION) => {
    ...
    if (left.channel !== right.channel) return channelRank[left.channel] > channelRank[right.channel];
  ...
  function renderAvailableUpdate() {
    ...
    if (!isNewerVersion(latestAvailableVersion)) return;
  ...
  function startRowPulse(call, kind) {
    ...
    window.setTimeout(() => {
      if (rowPulses.get(call) !== pulse) return;
  ...
  function observeStationTransitions(stations) {
    ...
    if (stationTransitionBaseline) {
      hiddenCalls.forEach(call => {
        if (!next.has(call)) hiddenAwayCalls.add(call);
      ...
      next.forEach((current, call) => {
        ...
        if (current.checkedState === true && prior?.checkedState !== true) scrollActiveAfterRender = true;
        ...
        if (prior?.checkedState === false && current.checkedState === true) {
          ...
          if (canManageStations()) publishSharedTags(call);
        }
      ...
      if (selectedNextCall && next.get(selectedNextCall)?.checkedState !== true) {
        selectedNextCall = "";
        if (canManageStations()) publishSharedSelection();
      }
    }
  ...
  function pulsePresentation(call) {
    ...
    if (!pulse) return { className: "", style: "" };
  ...
  async function fetchNet() {
    ...
    if (!response.ok) throw new Error(`Net data unavailable (${response.status})`);
  ...
  async function runCommand(command, finalMessage = "") {
    ...
    try {
      ...
      if (!action) throw new Error("That logger action is not supported.");
      const payload = { action };
      if (["f"].includes(verb)) payload.frequency = rawValue;
      else if (rawValue) payload.callSign = normalizeCall(rawValue);
      ...
      if (!response.ok) throw new Error(data.errorMessage || `${response.status} ${response.statusText}`);
      setStatus(finalMessage || commandMessage(command, "success"), "success");
      if (canManageStations()) local.sharedUpdatedAt = Date.now();
      scheduleRefresh(350);
      ...
      if (pendingCallAcquired) pendingCheckStateCalls.end(pendingCall);
    }
  }
  ...
  function publishSharedSnapshotSafely() {
    ...
    if (loggerStateSaveTimer) window.clearTimeout(loggerStateSaveTimer);
  ...
  async function lookupQrz(callSign, options = {}) {
    ...
    if (!call) return setStatus("Enter a callsign first.", "error");
    if (!silent) setStatus(`Looking up ${call} on QRZ…`, "working");
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.errorMessage || `Server QRZ lookup failed (${response.status}).`);
      const result = data.message || {};
      ...
      if (!profile || typeof profile !== "object") throw new Error(`Server QRZ outcome: ${qrzStatus}.`);
      ...
      if (!silent) setStatus(`QRZ lookup failed for ${call}: ${error.message || String(error)}`, "error");
  ...
  function queueMissingQrzPhotos() {
    ...
    const priority = station => {
      if (normalizeCall(station.callSign) === selfCall()) return 0;
      if (station.checkedState === true && station.role === "netcontrol") return 1;
      if (station.checkedState === true && station.role === "netlogger") return 2;
      if (station.checkedState === true && station.role === "netrelay") return 3;
      if (station.checkedState === true) return 4;
      if (station.checkedState === false) return 5;
      return 6;
    };
    [...latestStations].sort((left, right) => priority(left) - priority(right)).forEach(station => {
      ...
      const isFresh = isQrzNameFresh(String(saved.qrzPhotoOutcome || ""), saved.qrzNameVersion,
        QRZ_NAME_VERSION, saved.qrzCheckedAt, Date.now());
      ...
      if (!call || isFresh || retryBlocked || alreadyQueued) return;
  ...
  async function processQrzLookupQueue() {
    ...
    if (qrzLookupRunning || !qrzLookupQueue.length) return;
  ...
  function deduplicateChatMessages() {
    ...
    if (Array.isArray(chat?.messages)) {
      ...
      const uniqueMessages = chat.messages.filter(message => {
        ...
        if (!id) return true;
        if (seenDataIds.has(id)) return false;
        seenDataIds.add(id);
        return true;
      });
      if (uniqueMessages.length !== chat.messages.length) chat.messages = uniqueMessages;
    }
    const seen = new Set();
  ...
  function refreshHelperPeers() {
    ...
    latestStations.forEach(station => {
      if (typeof station.checkedState !== "boolean") return;
      const call = normalizeCall(station.callSign);
      if (!call) return;
    ...
    [...helperPeers.keys()].forEach(call => {
      if (!loggedCalls.has(call)) helperPeers.delete(call);
    });
  }
  ...
  function decodeHelperPrivate(text) {
    if (typeof text !== "string" || !text.startsWith(PRIVATE_CHAT_PREFIX)) return null;
    try {
      const message = JSON.parse(text.slice(PRIVATE_CHAT_PREFIX.length));
      if (message?.version !== 1 || message?.net_id !== npid || !["presence", "private"].includes(message?.action)) return null;
      if (!message.id || !Number.isFinite(Number(message.timestamp)) || !message.payload || typeof message.payload !== "object") return null;
      return message;
    } catch {
      return null;
    }
  ...
  async function publishHelperPrivate(action, payload) {
    ...
    try { return Boolean(await relayClient.publish({ text }, makeLocalId("helper"))); }
    catch { return false; }
  }

  function publishHelperPresence() {
    const identity = currentRelayIdentity();
    if (!identity || !relayClient) return;
  ...
  function receiveHelperPrivate(frame, message) {
    ...
    if (!senderCall || !senderUserId) return;
    ...
    if (message.action === "private") {
      ...
      if ((toCall !== selfCall() && (!identity || toUserId !== identity.userId)) || !body) return;
  ...
  function renderHelperChatUiUnsafe() {
    ...
    if (!selector || !summary || !options) return;
    ...
    const peerOptions = (items, available) => items.map(peer =>
      `<button type="button" role="option" class="nch-private-peer-option nch-private-peer-${available ? "available" : "unavailable"}" data-private-choice="${escapeHtml(peer.callSign)}" aria-selected="${peer.callSign === privateChatTarget ? "true" : "false"}"${available ? "" : " disabled"}>
    ...
    if (privateMessages) {
      ...
      if (privateMessages.innerHTML !== html) privateMessages.innerHTML = html;
      privateMessages.dataset.threadCall = privateChatTarget;
      if (switchingThread || wasAtBottom) privateMessages.scrollTop = privateMessages.scrollHeight;
      else privateMessages.scrollTop = priorScrollTop;
    }
  }
  ...
  function chatMessagePlainText(message, maxLength = 2000) {
    ...
    if (body) return String(body.innerText || body.textContent || "").replace(/\s+/g, " ").trim().slice(0, maxLength);
  ...
  function renderPinnedChatStrip() {
    ...
    if (strip.parentNode !== parent || (before && strip.nextSibling !== before)) {
      if (before?.parentNode === parent) parent.insertBefore(strip, before);
      else if (parent === chat || chat.contains(parent)) parent.appendChild(strip);
    }
  ...
  function togglePinnedChatMessage(message, explicitId = "") {
    ...
    if (!message.dataset.messageId && !message.dataset.id) message.dataset.messageId = id;
  ...
  function normalizeLatestChatPrompt() {
    ...
    const hasVisibleMessage = [...chat.querySelectorAll(".chat-message, [data-message-id]")]
      .some(message => {
        if (message.hidden || message.closest("[hidden]")) return false;
        const text = chatMessagePlainText(message, 400);
        if (!text || text.includes("~NCHSYNC1~")) return false;
  ...
  function normalizeChatDisplay(roots = [nativeChat()], includeGlobalChecks = true) {
    ...
    roots.flatMap(root => matchingChatElements(root,
      ".chat-message-author, .chat-message-sender, .chat-sender, .chat-author, .chat-user, .chat-user-name, .chat-username"
    )).forEach(element => {
    ...
    roots.flatMap(root => matchingChatElements(root, ".chat-message *")).forEach(element => {
      if (element.childElementCount || element.matches(".chat-text, .chat-message-content")) return;
    ...
    roots.flatMap(root => matchingChatElements(root, ".chat-message-content img")).forEach(image => {
      ...
      if (!container || container.querySelector(":scope > .nch-chat-download")) return;
  ...
  function safeNormalizeChatDisplay(records = null) {
    ...
    if (resumeObserver) observer.disconnect();
    try {
      ...
      const removedMessage = mutationRecords.some(record => [...record.removedNodes].some(node =>
        node instanceof Element && (node.matches(".chat-message, [data-message-id]") || node.querySelector(".chat-message, [data-message-id]"))
      ));
      const addedMessage = roots.some(node =>
        node.matches(".chat-message, [data-message-id]") || node.querySelector(".chat-message, [data-message-id]")
      );
      ...
      if (!includeGlobalChecks && addedMessage && chat?.querySelector(".nch-stale-latest")) normalizeLatestChatPrompt();
    } catch (error) {
  ...
  function openChatImage(event) {
    ...
    if (openPin && chatImageHost?.contains(openPin)) {
      ...
      event.preventDefault();
      event.stopImmediatePropagation();
      if (expandedPinnedChatIds.has(id)) expandedPinnedChatIds.delete(id);
      else expandedPinnedChatIds.add(id);
      renderPinnedChatStrip();
      return;
    }
  ...
  function currentRelayIdentity() {
    ...
    if (!/^[A-Z0-9/.-]{1,24}$/.test(callSign) || !/^[A-Za-z0-9_-]{1,64}$/.test(userId)) return null;
  ...
  async function resolvedAvatarSource(candidate) {
    if (resolvedAvatarDataUrls.has(candidate)) return resolvedAvatarDataUrls.get(candidate);
    if (Number(avatarFailureRetryAt.get(candidate) || 0) > Date.now()) return "";
    if (avatarSourceCache.has(candidate)) return avatarSourceCache.get(candidate);
    ...
    if (avatarSourceCache.get(candidate) === pending) avatarSourceCache.delete(candidate);
    if (source) {
      avatarFailureRetryAt.delete(candidate);
      setBoundedCache(resolvedAvatarDataUrls, candidate, source);
    } else setBoundedCache(avatarFailureRetryAt, candidate, Date.now() + AVATAR_TRANSIENT_RETRY_MS);
    return source;
  }
  ...
  function hydrateAvatars(root = panel) {
    root?.querySelectorAll("img.nch-avatar[data-qrz-photo]").forEach(image => {
      ...
      if (!candidate || image.dataset.loading === "true") return;
      image.onerror = () => {
        image.onerror = null;
      ...
      if (resolvedAvatarDataUrls.get(candidate) === image.src) return;
  ...
  function renderNetEditConnections() {
    ...
    netEditConnections.forEach((connection, index) => {
      const card = document.createElement("div"); card.className = "nch-net-connection-card";
      const type = document.createElement("select"); type.className = "nch-select"; type.setAttribute("aria-label", "Connection Type");
      ...
      const remove = document.createElement("button"); remove.type = "button"; remove.textContent = "Remove"; remove.addEventListener("click", () => { netEditConnections.splice(index, 1); renderNetEditConnections(); });
      const header = document.createElement("div"); header.className = "nch-net-connection-header"; header.append(type, remove); card.append(header);
      const fields = document.createElement("div"); fields.className = "nch-net-connection-fields";
      (NET_EDIT_CONNECTION_FIELDS[connection.type] || []).forEach(field => {
        ...
        if (field.options) field.options.forEach(value => input.add(new Option(value, value)));
        input.value = connection[field.key] || ""; input.required = Boolean(field.required);
        input.addEventListener("change", () => { if (input.value) connection[field.key] = input.value; else delete connection[field.key]; if (field.key === "operation") { if (input.value === "Simplex") delete connection.offset; renderNetEditConnections(); } });
        input.addEventListener("input", () => { if (!field.options) { if (input.value) connection[field.key] = input.value; else delete connection[field.key]; } });
        label.append(input); fields.append(label);
      });
      card.append(fields); container.append(card);
    });
    if (!netEditConnections.length) { const empty = document.createElement("p"); empty.className = "nch-net-connections-empty"; empty.textContent = "No connections added."; container.append(empty); }
  }
  ...
  async function correctCallsign(oldCall, newCall, station, editedDetails) {
    ...
    if (!confirm(`Correct ${oldCall} to ${newCall}? The original will be unchecked and the corrected callsign will be logged.`)) return false;
    ...
    if (station?.role === "netlogger") roleRestored = await runCommand(`l ${newCall}`);
    if (station?.role === "netrelay") roleRestored = await runCommand(`r ${newCall}`);
  ...
  async function saveEditModal() {
    ...
    if (!call) return setStatus("Enter a callsign.", "warning");
    const editedDetails = {
      ...current,
  ...
  function ordered(stations, key) {
    ...
    calls.forEach(call => { if (!local[key].includes(call)) local[key].push(call); });
    return [...stations].sort((a, b) =>
      local[key].indexOf(normalizeCall(a.callSign)) - local[key].indexOf(normalizeCall(b.callSign))
    );
  }
  ...
  function markIo(callSign, state) {
    ...
    if (state) local.ioCalls.push(call);
    storageSet();
  }
  ...
  function setRecheck(callSign, state, persist = true) {
    ...
    if (state) local.recheckCalls.push(call);
  ...
  function restoreHiddenCall(callSign, publish = true) {
    const call = normalizeCall(callSign);
    if (!hiddenCalls.has(call)) return false;
    hiddenCalls.delete(call);
    hiddenAwayCalls.delete(call);
    local.hiddenCalls = [...hiddenCalls];
    storageSet();
    if (publish) publishSharedVisibility(call, false);
    return true;
  }

  async function changeStationRole(callSign, desiredRole) {
    if (!canManageStations()) return false;
    const call = normalizeCall(callSign);
    const station = latestStations.find(item => normalizeCall(item.callSign) === call);
    if (!station || !["netuser", "netrelay", "netlogger", "netcontrol"].includes(desiredRole)) return false;
    ...
    if (currentRole === desiredRole) return true;
    if (currentRole === "netcontrol" && call === selfCall()) {
      setStatus("Use NCO handoff on another active station to leave the NCO role.", "warning");
      return false;
    }
    let command = "";
    if (desiredRole === "netlogger") command = `l ${call}`;
    if (desiredRole === "netrelay") command = `r ${call}`;
    if (desiredRole === "netcontrol") {
      if (!confirm(`Make ${call} the NCO? This transfers net control to ${call} and makes you the Logger.`)) return false;
      command = `handoff ${call}`;
    }
    if (desiredRole === "netuser") {
      if (currentRole === "netlogger") command = `l ${call}`;
      if (currentRole === "netrelay") command = `r ${call}`;
      if (currentRole === "netuser") return true;
    }
    if (!command || !await runCommand(command)) return false;
  ...
  async function setRowTag(callSign, tag) {
    ...
    if (tag === "inOut") {
      ...
      if (active) markIo(call, false);
      else if (station.checkedState === true) {
        if (!await runCommand(`o ${call}`)) return false;
        clearCheckoutAlerts(call);
        markIo(call, true);
      } else if (station.checkedState === null) {
        await lookupAndCheckIn(call, null, "io");
      } else markIo(call, true);
    } else if (tag === "recheck") {
      if (station.checkedState !== true) return false;
  ...
  function setSpecialGuest(callSign, state) {
    ...
    if (Boolean(current.specialGuest) === nextState) return false;
  ...
  function clearLocalStationTags(callSign, tags) {
    ...
    for (const tag of tags) {
      if (tag === "recheck") {
        if (!current.recheck) continue;
        setRecheck(call, false, false);
      } else if (tag === "inOut") {
        if (!local.ioCalls.includes(call)) continue;
        local.ioCalls = local.ioCalls.filter(item => item !== call);
      } else {
  ...
  async function clearRowTag(callSign, tag) {
    ...
    const station = latestStations.find(item => normalizeCall(item.callSign) === call);
    if (!station) return false;
    if (tag === "highlight") return station.highlight
  ...
  function hasClearableStationTags(callSign) {
    ...
    const call = normalizeCall(callSign);
    const station = latestStations.find(item => normalizeCall(item.callSign) === call);
    if (!station?.checkedState) return false;
    const details = detailsFor(call);
  ...
  async function clearStationTags(callSign) {
    const call = normalizeCall(callSign);
    if (!hasClearableStationTags(call)) return false;
    const station = latestStations.find(item => normalizeCall(item.callSign) === call);
    ...
    if (changed) setStatus(`Tags cleared for ${call}.`, "success");
    return changed;
  }
  ...
  function roleBadge(station, details, call) {
    ...
    if (station.checkedState !== true || role === "netuser") return "";
  ...
  function tagBadges(call, station, details) {
    if (station.checkedState === false) return "";
    const tags = [];
    if (station.highlight && station.checkedState === true) tags.push(statusTag(call, "highlight", "!", "nch-tag-highlight"));
    if (details.mobile) tags.push(statusTag(call, "mobile", "Mobile", "nch-tag-mobile"));
    if (details.shortTime) tags.push(statusTag(call, "shortTime", "Short Time", "nch-tag-short-time"));
    if (details.portable) tags.push(statusTag(call, "portable", "Portable", "nch-tag-portable"));
    if (station.checkedState === false && local.ioCalls.includes(call)) tags.push(statusTag(call, "inOut", "I/O", "nch-tag-io"));
    if (station.checkedState === true && details.recheck) tags.push(statusTag(call, "recheck", "RECHECK", "nch-tag-recheck"));
    if (station.checkedState === true && details.notResponding) tags.push(statusTag(call, "notResponding", "NO REPLY", "nch-tag-no-reply"));
    if (station.checkedState === true && details.neededNext) tags.push(statusTag(call, "neededNext", "NEEDED NEXT", "nch-tag-needed"));
    if (station.checkedState === true && details.skipped) tags.push(statusTag(call, "skipped", "SKIP", "nch-tag-skip"));
    return tags.join("");
  }
  ...
  const shortcutHint = key => /Mac|iPhone|iPad|iPod/i.test(
  ...
  function stationActionTray(station, details, call, busy, modal = false) {
    ...
    if (manager && active) {
      ...
      if (!isNco) status.push(trayButton("In & Out", 'data-set-tag="inOut"', "nch-inout-toggle", local.ioCalls.includes(call)));
      if (active) status.push(trayButton("Recheck", 'data-set-tag="recheck"', "nch-recheck-toggle", details.recheck));
      if (!specialRole || details.specialGuest) status.push(trayButton("Special Guest", `data-special-guest="${escapeHtml(call)}"`, "nch-guest-toggle", details.specialGuest, "G"));
      ...
      roleChoices.forEach(([role, label]) => {
        ...
        if (allowed) roles.push(trayButton(label, `data-set-role="${role}"`, `nch-role-choice nch-role-choice-${role}`, selected));
      });
    }

    if (manager && !active) {
      const compactManage = [];
      if (checkedOut) compactManage.push(trayButton("Check In", `data-row-checkin="${escapeHtml(call)}"`, "nch-checkin-btn"));
      if (lurker) compactManage.push(trayButton(busy ? "Checking In…" : "Check In", `data-add-lurker="${escapeHtml(call)}"${busy ? " disabled" : ""}`, "nch-add-lurker"));
      if (!specialRole) compactManage.push(trayButton("Delete", `data-delete="${escapeHtml(call)}"`, "nch-delete-btn"));
    ...
    if (manager) manage.push(trayButton("Edit Station", `data-edit="${escapeHtml(call)}"`, "nch-edit-btn", false, "E"));
    ...
    if (manager && checkedOut) manage.push(trayButton("Check In", `data-row-checkin="${escapeHtml(call)}"`, "nch-checkin-btn"));
    if (manager && lurker) manage.push(trayButton(busy ? "Checking In…" : "Check In", `data-add-lurker="${escapeHtml(call)}"${busy ? " disabled" : ""}`, "nch-add-lurker"));
    if (manager && !specialRole) manage.push(trayButton("Delete", `data-delete="${escapeHtml(call)}"`, "nch-delete-btn"));
  ...
  function stationActionToggle(station, call) {
    ...
    if (!touchActions || (station.checkedState !== true && !canManageStations())) return "";
  ...
  function hotkeyTypingContext(event) {
    ...
    if (candidates.some(node => {
      ...
      return node.isContentEditable || node.matches(
        "input, textarea, select, form, [contenteditable]:not([contenteditable='false']), [role='textbox'], [role='dialog'], [role='alertdialog'], hl-chat"
      );
    })) return true;
  ...
  function selectedHotkeyRow() {
    ...
    if (!call || !panel) return null;
    const station = latestStations.find(item => normalizeCall(item.callSign) === call);
    if (station?.checkedState !== true) return null;
  ...
  function moveSelectedHotkey(direction) {
    if (!panel || !canManageStations()) return false;
    ...
    if (nextIndex < 0 || nextIndex >= calls.length) return false;
  ...
  async function handleActionHotkey(event) {
    ...
    if (!selector && !clearTags && !moveDirection) return;
    if (hotkeyTypingContext(event)) return;
    if (moveDirection) {
      if (!moveSelectedHotkey(moveDirection)) return;
      event.preventDefault();
    ...
    if (!selected) return;
    if (clearTags) {
      if (!hasClearableStationTags(selected.call)) return;
      event.preventDefault();
      event.stopPropagation();
  ...
  const slashComposer = () => nativeChat()?.querySelector(
  ...
  function slashCommandsForRole() {
    if (isNcoUser()) return SLASH_COMMANDS;
  ...
  function renderSlashSuggestions() {
    ...
    const commands = slashCommandsForRole().filter(command =>
      !query || command.name.startsWith(query) || command.aliases.some(alias => alias.startsWith(query))
    );
  ...
  function setupSlashSuggestions() {
    ...
    if (input.dataset.nchSuggestionsBound === "true") return;
  ...
  function slashTarget(rawCall, selectedFallback = true) {
    ...
    if (!call || !validCall(call)) return null;
    const station = latestStations.find(item => normalizeCall(item.callSign) === call);
    if (!station) return null;
    const row = panel?.querySelector(`.nch-row[data-call='${CSS.escape(call)}']`) || null;
    if (!explicit && (station.checkedState !== true || !row?.closest("[data-role='active']"))) return null;
  ...
  function showSlashHelp() {
    ...
    if (section) section.innerHTML = slashHelpHtml();
  ...
  async function runSlashCommand(rawText) {
    ...
    if (["in", "hi"].includes(command.name)) {
      if (args.length !== 1 || !validCall(normalizeCall(args[0]))) return usage();
      if (!canManageStations()) return slashDenied();
      const call = normalizeCall(args[0]);
      setSlashComposerText("");
      const succeeded = await lookupAndCheckIn(call);
      if (succeeded && command.name === "hi") await setStationInteraction(call, "highlight", true);
      return;
    }
    if (command.name === "li") {
      if (args.length) return usage();
      ...
      for (const call of calls) {
        ...
        if (current?.checkedState !== null || hiddenCalls.has(call)) continue;
        if (await lookupAndCheckIn(call)) affected += 1;
      }
    ...
    if (command.name === "f") {
      ...
      if (!isNcoUser()) return slashDenied();
      const frequency = String(args[0] || "").trim();
      if (!frequency || frequency.length > 40 || /[\u0000-\u001f/]/.test(frequency)) return usage();
    ...
    if (command.name === "w") {
      ...
      if (succeeded) setSlashComposerText("");
      return;
    }
    if (args.length > 1 || (args[0] && !validCall(normalizeCall(args[0])))) return usage();
    const target = slashTarget(args[0], true);
    if (!target) return usage();
    if (command.name === "note") {
      const button = slashRowButton(target, ".nch-note-action");
      if (!button) return slashDenied();
      setSlashComposerText("");
      button.click();
      return;
    }
    ...
    if (!button || button.disabled) return slashDenied();
    setSlashComposerText("");
    button.click();
  }

  function handleSlashBridgeEvent(event) {
    if (!panel || !nativeChat()?.classList.contains("nch-helper-slash-owned")) return;
    let payload;
  ...
  function registerSlashBridgeHandler() {
    if (slashBridgeHandlerRegistered) return;
  ...
  function positionStationActionModal() {
    ...
    if (!modal.classList.contains("nch-nonactive-station-action-modal") || !currentLayoutContext.startsWith("tablet")) return;
    ...
    if (!(sourceModule instanceof HTMLElement)) return;
  ...
  function updateStationGroup(element, html) {
    if (!element || renderedStationGroupHtml.get(element) === html) return false;
  ...
  function renderQueue() {
    ...
    const focusedNote = document.activeElement?.matches?.("[data-note-input]")
      ...
      : null;
    if (focusedNote) noteDrafts.set(focusedNote.call, focusedNote.value);
    const visibleStations = latestStations.filter(s => !hiddenCalls.has(normalizeCall(s.callSign)));
    if (pinnedActionCall && !visibleStations.some(s => normalizeCall(s.callSign) === pinnedActionCall)) pinnedActionCall = "";
    if (touchInlineActionCall && !visibleStations.some(s => normalizeCall(s.callSign) === touchInlineActionCall && s.checkedState !== true)) touchInlineActionCall = "";
    panel.classList.toggle("nch-has-pinned-actions", Boolean(pinnedActionCall));
    ...
    activeOrdered.forEach(station => {
      if (station.role === "netcontrol") activeGroups.ncos.push(station);
      else if (station.role === "netlogger") activeGroups.loggers.push(station);
      else if (station.role === "netrelay") activeGroups.relays.push(station);
      else if (detailsFor(station.callSign).specialGuest) activeGroups.guests.push(station);
      else activeGroups.active.push(station);
    });
    ...
    if (activeHeadingNote) activeHeadingNote.textContent = canManageStations()
    ...
    if (checkedOutHeadingNote) checkedOutHeadingNote.textContent = canManageStations() ? "drag to reorder" : "read only";
    ...
    visibleStations.forEach(station => {
      if (typeof station.checkedState === "boolean") loggedCount += 1;
      if (station.checkedState === true) {
        activeCount += 1;
        if (detailsFor(station.callSign).recheck) recheckCount += 1;
      } else if (station.checkedState === false) checkedOutCount += 1;
    });
  ...
  function orientRowActions(row) {
    if (!panel || !(row instanceof Element) || !row.classList.contains("nch-row")) return;
    ...
    if (wouldClipBelow && (canFitAbove || aboveSpace >= belowSpace)) row.classList.add("nch-actions-above");
    if (hidden) {
  ...
  function clearActionOrientationRows() {
    panel?.querySelectorAll(".nch-actions-above").forEach(row =>
      row.classList.remove("nch-actions-above")
    );
  }
  ...
  function clearPinnedStationAction(call) {
    ...
    if (!pinnedActionCall || pinnedActionCall !== normalizedCall) return false;
    pinnedActionCall = "";
  ...
  function moveDragged(targetCall, targetGroup, after = false) {
    ...
    if (!dragging || dragging.group !== targetGroup || dragging.call === targetCall) return;
    ...
    if (targetGroup === "order") local.manualOrder = true;
    storageSet();
    publishSharedMove(targetGroup);
    renderQueue();
  ...
  function reconcileStationMetadata(stations) {
    stations.forEach(station => {
      ...
      if (sharedProfiles[call]) delete sharedProfiles[call];
    });
  }
  ...
  function viewerItems() {
    return latestStations.filter(station => !hiddenCalls.has(normalizeCall(station.callSign))).map(station => {
      ...
      if (call === selfCall()) actions.push({ action_id: "toggle_own_hand", label: station.hand ? "Lower own hand" : "Raise own hand", owner_mode: modeId(), enabled: true });
      return {
  ...
  function ensureViewer() {
    if (viewer || !globalThis.NCOHelperViewer) return viewer;
  ...
  const profileAuthorityRank = (role, origin) => {
    ...
    if (role === "netlogger") return origin === "manual" ? 20 : 10;
    return 0;
  };
  const profileCandidateWins = (saved, field, role, origin, timestamp, messageId) => {
    ...
    if (saved?.[`${field}Cleared`]) {
      if (Number(timestamp) !== currentTime) return Number(timestamp) > currentTime;
  ...
  function applyProfileCandidate(callSign, profile, role, timestamp = Date.now(), messageId = "") {
    const call = normalizeCall(callSign);
    if (!call || !["netcontrol", "netlogger"].includes(role)) return { accepted: {}, rejected: [...PROFILE_FIELDS] };
    ...
    if (!Object.keys(accepted).length) return { accepted, rejected };
  ...
  function sharedSnapshot() {
    ...
    latestStations.forEach(station => {
      ...
      if (Object.values(tags).some(Boolean) || profile) details[call] = { tags, ...(profile ? { profile } : {}) };
    });
    const updatedAt = Number(local.sharedUpdatedAt) || Date.now();
    if (!local.sharedUpdatedAt) local.sharedUpdatedAt = updatedAt;
    return {
  ...
  function applySnapshotProfiles(payload, update) {
    Object.entries(payload.details || {}).forEach(([rawCall, value]) => {
      ...
      for (const field of SHARED_PROFILE_FIELDS) {
        ...
        }, role, Number(value.profile[`${field}ChangedAt`]) || Number(payload.updated_at) || Date.now(),
        value.profile[`${field}ChangeId`] || `snapshot-${update.envelope?.message_id || "state"}`);
      }
    });
  }
  ...
  function applySharedUpdate(update) {
    ...
    if (action === "tags") {
      ...
      if (payload.group === "order") local.manualOrder = Boolean(payload.manual_order);
    } else if (action === "visibility") {
      const call = normalizeCall(String(payload.entity_id).replace(/^station:/, ""));
      if (payload.hidden) hiddenCalls.add(call);
      else hiddenCalls.delete(call);
      local.hiddenCalls = [...hiddenCalls];
      ...
      ["order", "checkedOutOrder", "lurkerOrder"].forEach(key => { if (Array.isArray(payload[key])) local[key] = payload[key].map(normalizeCall); });
      ...
      Object.entries(payload.details || {}).forEach(([rawCall, value]) => {
        const call = normalizeCall(rawCall);
        if (!call || !value || typeof value !== "object") return;
  ...
  const publishSharedProfile = (call, profile) => {
    if (!canManageStations() || !profile || !Object.keys(profile).length) return;
    local.sharedUpdatedAt = Date.now();
    publishSharedSnapshotSafely();
  };
  ...
  async function sendSyncText(message) {
    if (!relayClient || typeof message !== "string") return false;
    try {
      const envelope = syncEngine?.decode(message);
      if (!envelope?.message_id) return false;
      return relayClient.publish({ text: message }, envelope.message_id);
    } catch { return false; }
  }

  function stopSync() {
    if (syncSnapshotRequestTimer) window.clearTimeout(syncSnapshotRequestTimer);
  ...
  function applyRoleUi() {
    ...
    if (closeButton) closeButton.hidden = !isNcoUser();
  }
  ...
  function normalizeModuleLayout(source = local.moduleLayout) {
    ...
    if (!hasCurrentGrid && !hasV2Grid && hasLegacy) {
      ...
      legacyOrder.forEach(id => {
        ...
        const w = Math.min(GRID_COLUMNS, Math.max(MIN_MODULE_COLUMNS,
          legacySpan ? Math.round(legacySpan * GRID_COLUMNS / LEGACY_GRID_COLUMNS) : fallback.w));
    ...
    MODULE_IDS.forEach(id => {
      ...
      if (hasCurrentGrid && candidate.items[id] && typeof candidate.items[id] === "object") supplied = candidate.items[id];
      if (hasV3Grid && candidate.items[id] && typeof candidate.items[id] === "object") {
        ...
        if (oldDefault && ["x", "y", "w", "h"].every(key => Number(supplied[key]) === oldDefault[key])) supplied.h = 4;
      }
    ...
    if (currentLayoutContext === "tabletPortrait" && moduleAvailable("controls")) {
      ...
      // to Chat/Active instead of an old saved h=6+ invisibly blocking that row.
      items.controls.h = 5;
      items.controls.y = 0;
    }
  ...
  function canonicalizeReadOnlyTop(layout, force = false) {
    ...
    if (moduleAvailable("controls") || currentLayoutContext !== "desktop") return normalized;
    const lurkers = normalized.items.lurkers;
    const checkedOut = normalized.items.checkedOut;
    if (!force && (lurkers.y !== 0 || checkedOut.y !== 0)) return normalized;
  ...
  function restoreModuleLayout(source) {
    ...
    if (gridLayoutIsCollisionFree(normalized)) return normalized;
  ...
  function inheritedViewerLayoutForContext(context) {
    if (!["tabletPortrait", "tabletLandscape"].includes(context)) return rawDefaultModuleLayoutFor("netuser", context);
  ...
  function legacyLayoutShouldResetForRole(candidate, role, context) {
    return shouldResetLegacyLoggerLayout(
      candidate, role, rawDefaultModuleLayoutFor("netcontrol", context),
      rawDefaultModuleLayoutFor("netuser", context), inheritedViewerLayoutForContext(context)
    );
  }
  ...
  function migrateLegacyLayoutsForRole(role = currentUserRole) {
    ...
    if (["nco", "logger"].includes(loggerLayoutRole(role))
      ...
      // Replace only the exact short-lived seven-row default. Custom layouts
      // remain untouched, including layouts produced by no-peer-repacking drag.
  ...
  function activateLayoutRole(role, previousRole = "") {
    if (layoutRoleResolved && previousRole) saveActiveLayoutContext(previousRole);
  ...
  function switchLayoutContext(nextContext) {
    ...
    if (label) label.textContent = layoutContextLabel(nextContext);
  ...
  function applyModuleLayout() {
    ...
    const focusedNote = document.activeElement?.matches?.("[data-note-input]")
      ...
      : null;
    if (focusedNote) noteDrafts.set(focusedNote.call, focusedNote.value);
    // Rendering is not a layout mutation. Recovery is confined to restoreModuleLayout();
    // all interactive changes below replace only the module being manipulated.
    local.moduleLayout = normalizeModuleLayout(local.moduleLayout);
    if (!panel.querySelector("[data-role='dashboard']")) return;
    ...
    panel.querySelectorAll("[data-toggle-module]").forEach(button => {
      ...
      if (state) state.textContent = !available ? "Unavailable" : visible ? "On" : "Off";
    });
    syncNativeChatVisibility();
    window.requestAnimationFrame(positionNativeChat);
    if (focusedNote) {
  ...
  function setModuleCollapsed(id, collapsed) {
    ...
    if (!collapsed && !gridLayoutIsCollisionFree(layout)) return;
  ...
  function resizeModuleBy(id, widthDelta, heightDelta) {
    ...
    if (!gridLayoutIsCollisionFree(layout)) return;
    local.moduleLayout = layout;
    storageSet();
    applyModuleLayout();
  }
  ...
  function gridMetrics(dashboard) {
    ...
    const nearestRowStart = (pixel, maximumStart) => {
      ...
      for (let row = 0; row <= maximumStart; row += 1) {
        if (Math.abs(rowStarts[row] - pixel) < Math.abs(rowStarts[best] - pixel)) best = row;
      }
      return best;
    };
    const nearestRowEnd = (pixel, minimumEnd, maximumEnd) => {
      ...
      for (let row = minimumEnd; row <= maximumEnd; row += 1) {
        if (Math.abs(rowEnds[row] - pixel) < Math.abs(rowEnds[best] - pixel)) best = row;
      }
      return best;
    };
  ...
  function collisionFreeModulePosition(source, id, requestedX, requestedY, priorX, priorY) {
    const layout = normalizeModuleLayout(source);
    const item = layout.items[id];
    return findLoggerGridItemPosition(
      layout, id, { x: requestedX, y: requestedY }, { x: priorX, y: priorY },
      { maxX: GRID_COLUMNS - item.w, maxY: layoutRows() - item.h }, visibleModuleIds(layout)
    );
  }
  ...
  function updateModulePointerDrag(event) {
    ...
    if (!modulePointerDrag.started && distance < (event.pointerType === "touch" ? 12 : 6)) return;
    if (!modulePointerDrag.started) {
  ...
  function stopModulePointerDrag(event) {
    ...
    if (finished.started && finished.previewLayout) local.moduleLayout = finished.previewLayout;
    clearModuleDrag();
    applyModuleLayout();
    if (finished.started) storageSet();
  }
  ...
  function resizeFromPointer(event) {
    if (!resizing?.moduleId || event.pointerId !== resizing.pointerId) return;
    const dashboard = panel?.querySelector("[data-role='dashboard']");
    if (!dashboard) return;
    const metrics = gridMetrics(dashboard);
    ...
    const candidate = replaceLoggerGridItem(
      resizing.originalLayout, resizing.moduleId, item, visibleModuleIds(layout)
    );
    if (!candidate) return;
  ...
  function addPanel() {
    ...
    panel.addEventListener("click", async event => {
      ...
      if (clickedRow?.dataset.tabletNonactive === "true" && !clickedInteractive) {
        // Tablet Lurker/Checked Out rows are display-only. Only their explicit menu button is interactive.
        clickedRow.blur?.();
        return;
      }
      ...
      if (clickedRow && !clickedInteractive && touchStationActions) {
        ...
        if (station?.checkedState !== true) {
          // Lurkers and checked-out rows use their explicit menu button on touch layouts.
          // The row itself intentionally has no click/tap action.
          return;
        }
      }
      ...
      if (target.dataset.rowCheckout) {
        ...
        if (succeeded) {
          clearCheckoutAlerts(call);
          markIo(call, false);
          if (station.highlight) setStationInteraction(call, "highlight", false);
          publishSharedTags(call);
          renderQueue();
        }
      }
      if (target.dataset.rowCheckin) await lookupAndCheckIn(target.dataset.rowCheckin);
      if (target.dataset.edit) openEditModal(target.dataset.edit);
      ...
      if (target.dataset.role === "add-net-connection") { netEditConnections.push({ type: "FM", operation: "Repeater" }); renderNetEditConnections(); }
      ...
      if (target.dataset.role === "menu-commands" || target.dataset.role === "commands-help") {
        showSlashHelp();
        const menu = panel.querySelector("[data-role='header-menu']");
        if (target.dataset.role === "menu-commands" && menu) menu.open = false;
      }
      ...
      if (target.dataset.role === "close-net") {
        ...
        if (isNcoUser() && modal) modal.hidden = false;
        syncNativeChatVisibility();
      }
      ...
      if (target.dataset.role === "confirm-close") {
        ...
        if (modal) modal.hidden = true;
        syncNativeChatVisibility();
        if (isNcoUser()) await runCommand("close");
      }
      if (target.dataset.role === "relay-save") {
        ...
        if (relayMenu) relayMenu.open = false;
        refreshRelaySetup();
      ...
      if (target.dataset.editorCommand) {
        ...
        const succeeded = await runCommand(`${command} ${call}`);
        if (succeeded) markIo(call, command === "io");
        renderQueue();
    ...
    const blockTabletNonactiveRowSurface = event => {
      ...
      if (!row || event.target.closest?.('.nch-station-action-toggle')) return;
      event.preventDefault();
      event.stopPropagation();
    };
    ...
    panel.addEventListener("pointerover", event => {
      const row = event.target.closest?.(".nch-row[data-call]");
      if (row && !pinnedActionCall) orientRowActions(row);
    });
    ...
    window.addEventListener("resize", () => {
      if (!pinnedActionCall || !panel) return;
    ...
    panel.addEventListener("focusout", event => {
      ...
      window.requestAnimationFrame(() => {
        if (noteInput.isConnected || editingNoteCall !== call) return;
    ...
    panel.addEventListener("keydown", async event => {
      ...
      if (editModal && !editModal.hidden && ["Enter", "Escape"].includes(event.key) && !event.isComposing && !event.repeat) {
        event.preventDefault();
        if (event.key === "Enter") await saveEditModal();
        else closeEditModal();
        return;
      }
    ...
    panel.addEventListener("pointerdown", event => {
      ...
      const moduleHandle = event.target.closest("[data-module-drag]");
      if (!moduleHandle || event.target.closest("button, input, select, textarea, a") || event.button !== 0) return;
      ...
      modulePointerDrag = {
        ...
        // Preserve the exact current layout at drag start. Running the global resolver here
        // can move unrelated modules before the user has moved the dragged module at all.
    ...
    panel.addEventListener("pointermove", event => {
      ...
      else updateModulePointerDrag(event);
    });
    panel.addEventListener("pointerup", event => {
      if (resizing) stopResizing(event);
      else stopModulePointerDrag(event);
    });
    panel.addEventListener("pointercancel", event => {
      if (resizing) stopResizing(event, true);
      else cancelModulePointerDrag(event);
    });
    ...
    panel.addEventListener("dblclick", event => {
      ...
      const layout = normalizeModuleLayout();
      const candidate = replaceLoggerGridItem(
        layout, id, defaultModuleLayoutForMode().items[id], visibleModuleIds(layout)
      );
      if (!candidate) return;
      local.moduleLayout = candidate;
      applyModuleLayout();
    ...
    panel.querySelectorAll("[data-role='menu-reset']").forEach(button => button.addEventListener("click", () => {
      ...
      const targetContext = orientation
        ...
        : "desktop";
      if (!confirm(`Reset only the ${layoutContextLabel(targetContext)} module layout?`)) return;
      ...
      if (targetContext === currentLayoutContext) local.moduleLayout = resetLayout;
      storageSet();
      applyModuleLayout();
      const menu = panel.querySelector("[data-role='header-menu']");
      if (menu) menu.open = false;
  ...
  async function refreshOnce() {
    ...
    try {
      ...
      if (requestSequence < refreshAppliedSequence) return;
      ...
      dockNativeChat();
      customizeNativeChat();
      if (panelWasMissing || remoteUiChanged) renderQueue();
      else syncViewer();
      if (focusedNote && editingNoteCall === focusedNote.call) {
        const input = panel.querySelector(`[data-note-input='${CSS.escape(focusedNote.call)}']`);
        input?.focus();
      ...
      if (panel) setStatus(error.message || String(error), "error");
    } finally {
      recordTiming(performanceStats.refreshMs, performance.now() - refreshStartedAt);
    }
  }

  const relayStorageGet = () => new Promise(resolve =>
  ...
  window.addEventListener("beforeunload", () => {
    ...
    if (loggerStateSaveTimer) clearTimeout(loggerStateSaveTimer);
