import { boundaryLimits, resizeSharedBoundary, sharedBoundaries } from './loggerSplitters.js';
export class LoggerSplitterControls {
    ...
    render() {
        ...
        for (const boundary of boundaries) {
            ...
            if (limits.min >= limits.max || this.options.allowed?.(boundary) === false)
                continue;
            ...
            const elements = [...boundary.before, ...boundary.after].map(id => this.dashboard.querySelector(`[data-module="${id}"]`));
        ...
        this.dashboard.querySelectorAll('[data-module]').forEach(element => {
    ...
    start(event, id) {
        ...
        const pane = this.dashboard.querySelector(`[data-module="${boundary.before[0]}"]`);
    ...
    finish(event, cancelled) {
        ...
        if (cancelled)
            this.options.apply(drag.layout);
        else
            this.options.save();
    }
    cancel() {
        if (!this.drag)
            return;
        this.finish({ pointerId: this.drag.pointerId }, true);
    }
    key(event, id) {
        const layout = this.options.read();
        const boundary = sharedBoundaries(layout, this.options.visible(layout)).find(item => item.id === id);
...
//# sourceMappingURL=loggerSplitterControls.js.map