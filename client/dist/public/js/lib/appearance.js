...
(() => {
    if (window.ncoLoggerAppearance) return;
    ...
    const normalizeAppearance = value =>
        typeof value === 'string' && allowedAppearances.has(value) ? value : DEFAULT_APPEARANCE;
    ...
    const applyAppearance = value => {
        ...
        window.dispatchEvent(
            new CustomEvent('ncoLogger:appearancechange', {
    ...
    window.addEventListener('storage', event => {
        if (event.key === STORAGE_KEY) applyAppearance(event.newValue);
    });
