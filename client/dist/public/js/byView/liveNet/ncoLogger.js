...
(() => {
    ...
    async function sendPrivateChatMessage() {
        ...
        if (!privatePeerAvailable(peer))
            return setStatus(`${targetCall} is not currently available for Live Logger private chat.`, "warning");
        if (!body)
            return;
        const identity = currentRelayIdentity();
        ...
        if (!sent)
            return setStatus("Private Live Logger chat could not send because the relay is not connected.", "error");
    ...
    async function openEditModal(callSign) {
        if (!canManageStations())
            return setStatus("Station editing is not available in this operating mode.", "warning");
        const call = normalizeCall(callSign);
    ...
    async function saveNetEditModal() {
        ...
        try {
            ...
            const data = await response.json().catch(() => ({}));
            if (!response.ok)
                throw new Error(data.errorMessage || `Live net update failed (${response.status}).`);
    ...
    async function changeStationRole(callSign, desiredRole) {
        ...
        if (desiredRole === "netcontrol") {
            ...
            latestStations = latestStations.map(station => {
                ...
                if (stationCall === ownCall)
                    return { ...station, role: "netlogger", level: 1 };
                if (stationCall === call)
    ...
    function touchRowDragCandidate(target) {
        if (!usesTouchStationInteractions() || !canManageStations())
            return null;
        if (target.closest("button, input, select, textarea, a, [contenteditable='true'], .nch-row-actions, .nch-inline-actions"))
            return null;
    ...
    function updateStationDropTarget(row, clientY) {
        ...
        if (scrollerBox && clientY < scrollerBox.top + 36)
            scroller.scrollTop -= 20;
        if (scrollerBox && clientY > scrollerBox.bottom - 36)
            scroller.scrollTop += 20;
        if (touchRowDrag)
            touchRowDrag.targetCall = normalizeCall(row.dataset.call);
        return true;
    }
    ...
    function finishTouchRowDrag(event, cancelled = false) {
        if (!touchRowDrag || !touchRowDragPoint(event))
            return false;
        ...
        if (completed) {
            ...
            if (!cancelled && targetCall)
                moveDragged(targetCall, group, after);
        }
        clearTouchRowDrag();
        return completed;
    }
    ...
    function resizeModuleBy(id, widthDelta, heightDelta) {
        if (id === "controls" || currentLayoutContext.startsWith("phone") || !MODULE_IDS.includes(id))
            return;
        ...
        if (!modulePlacementIsClear(layout, id))
            return;
    ...
    function previewModuleGrid(clientX, clientY) {
        ...
        if (!modulePlacementIsClear(candidate, moduleId)) {
            if (!(currentLayoutContext === "desktop" && moduleId === "controls"))
                return;
            ...
            if (!modulePlacementIsClear(candidate, overlapId) || !modulePlacementIsClear(candidate, moduleId))
                return;
        }
    ...
    function resizeFromPointer(event) {
        ...
        if (!modulePlacementIsClear(layout, resizing.moduleId))
            return;
        local.moduleLayout = layout;
        renderGridLayout(local.moduleLayout);
    }
    ...
    function syncModuleResizeAvailability() {
        ...
        panel?.querySelectorAll("[data-resize-module]").forEach(handle => {
            ...
            if (handle.dataset.resizeEdge === "se")
    ...
    function addPanel() {
      ...
      <div class="nch-body">
        ...
        panel.addEventListener("dragover", event => {
            ...
            if (!row || !updateStationDropTarget(row, event.clientY))
                return;
            event.preventDefault();
        });
