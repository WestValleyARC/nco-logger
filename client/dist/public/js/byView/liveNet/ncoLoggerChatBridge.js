(() => {
    ...
    function suppressNativeSlashUi(chat) {
        ...
        if (!composerText(input).trimStart().startsWith("/"))
            return;
    ...
    function disableSlashBridge() {
        ...
        if (slashChat) {
            ...
            if (originalExecuteSlashDescriptor)
                Object.defineProperty(slashChat, "executeSlashCommand", originalExecuteSlashDescriptor);
            else
                delete slashChat.executeSlashCommand;
        }
        slashChat = null;
    ...
    function bindSlashBridge() {
        if (!slashEnabled)
            return disableSlashBridge();
        const chat = document.querySelector("hl-chat");
        if (!chat || chat === slashChat)
            return;
        disableSlashBridge();
        slashChat = chat;
        ...
        const intercept = (event, source) => {
            const input = slashComposer(chat);
            if (!input || !dispatchSlash(chat, composerText(input), source))
                return false;
        ...
        chat.addEventListener("keydown", event => {
            const input = slashComposer(chat);
            if (!input || !event.composedPath().includes(input))
                return;
            ...
            if (event.key !== "Enter" || event.shiftKey || event.isComposing || !slash)
                return;
            if (event.repeat) {
                blockSlashSubmission(event);
                return;
        ...
        chat.addEventListener("click", event => {
            if (event.composedPath().some(node => node?.matches?.(".chat-send-btn")))
                intercept(event, "send");
        }, options);
    ...
    function attach() {
        ...
        if (!chat || !connection?.on) {
            if (boundChat && boundChat !== chat)
                detach();
            return;
        }
        if (chat === boundChat && connection === boundConnection)
            return;
        detach();
        boundChat = chat;
...
export {};
//# sourceMappingURL=ncoLoggerChatBridge.js.map